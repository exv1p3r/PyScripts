...
parser.add_option("-l", "--local_file", dest="local_file", help="LOCAL FILE NAME to save remote file to", metavar="LOCAL FILE NAME")
...
if options.username:
    try:
        ...
        print "[-] Login was failed: %s" % e
        sys.exit(1)
else:34
    try:
        ftp.login()
    except ftplib.error_perm, e:
        print "[-] Anonymous login was failed: %s" % e
        sys.exit(1)
try:
